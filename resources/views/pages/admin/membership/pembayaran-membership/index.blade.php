                    ...
                    <table id="selection-table" class="border border-neutral-200 rounded-lg border-separate w-full">
                        ...
                        <tbody>
                            @foreach ($anggotaMemberships as $index => $item)
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap flex gap-2">
                                        @if ($item->status_pembayaran === 'Lunas')
                                            <a href="{{ route('pembayaran_membership.notaPDF', $item->id) }}"
                                        ...
                                        @else
                                            @role('admin')
                                                <button type="button" title="Bayar"
                                                    data-modal-target="edit-popup-modal-{{ $item->id }}"
                                                    data-modal-toggle="edit-popup-modal-{{ $item->id }}"
                                    ...
                                    <td class="whitespace-nowrap">Rp
                                        {{ number_format($item->paketMembership->harga, 0, ',', '.') }}</td>
                                    ...
                                    <td class="whitespace-nowrap">Rp
                                        {{ number_format($item->pembayaranMemberships->sum('jumlah_bayar'), 0, ',', '.') }}
                                    </td>
                                ...
                                @role('admin')
                                    <div id="edit-popup-modal-{{ $item->id }}" tabindex="-1"
                                        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        <div class="rounded-2xl bg-white dark:bg-dark-2 max-w-[800px] w-full">
                                            <div
                                                class="py-4 px-6 border-b border-neutral-200 dark:border-neutral-700 flex items-center justify-between">
                                                <h1 class="text-xl">Tambah Pembayaran</h1>
                                                <button data-modal-hide="edit-popup-modal-{{ $item->id }}" type="button"
                                            ...
                                            <div class="p-6">
                                                <form action="{{ route('pembayaran_membership.tambahPembayaran', $item->id) }}"
                                                    method="POST" class="form-pembayaran" data-item-id="{{ $item->id }}">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                                                        ...
                                                        <div class="col-span-12 md:col-span-6">
                                                            ...
                                                            <input type="text" class="form-control bg-gray-50"
                                                                value="Rp {{ number_format($item->pembayaranMemberships->sum('jumlah_bayar'), 0, ',', '.') }}"
                                                                readonly>
                                                        </div>

                                                        {{-- Info Sisa Tagihan --}}
                                                        <div class="col-span-12">
                                                            <label class="form-label font-semibold text-danger-600">Sisa
                                                                Tagihan</label>
                                                            <input type="text"
                                                                class="modal_sisa_tagihan form-control bg-danger-50 border-danger-200 text-danger-600 font-bold text-lg"
                                                                value="Rp {{ number_format($item->total_biaya - $item->pembayaranMemberships->sum('jumlah_bayar'), 0, ',', '.') }}"
                                                                readonly>
                                                            <input type="hidden" id="sisa_tagihan_value_{{ $item->id }}"
                                                                class="sisa_tagihan_value"
                                                                value="{{ $item->total_biaya - $item->pembayaranMemberships->sum('jumlah_bayar') }}">
                                                        </div>
                                                        ...
                                                        <div class="col-span-12">
                                                            ...
                                                            <select name="metode_pembayaran" class="form-control" required>
                                                                ...
                                                                <option value="ewallet"
                                                                    {{ old('metode_pembayaran') == 'ewallet' ? 'selected' : '' }}>
                                                                    E-Wallet</option>
                                                            </select>
                                                        </div>
                                                        ...
                                                        <div class="col-span-12">
                                                            ...
                                                            <input type="number" name="jumlah_bayar"
                                                                id="jumlah_bayar_{{ $item->id }}"
                                                                class="modal_jumlah_bayar form-control" value="0"
                                                                min="0" required>
                                                            <small class="text-muted modal_warning_text"
                                                                style="display: none; color: #dc3545; margin-top: 4px;"></small>
                                                        </div>

                                                        {{-- Sisa Setelah Pembayaran Ini --}}
                                                        <div class="col-span-12">
                                                            <label class="form-label">Sisa Setelah Pembayaran Ini</label>
                                                            <input type="text" id="sisa_setelah_{{ $item->id }}"
                                                                class="modal_sisa_setelah form-control bg-success-50 border-success-200 text-success-600 font-semibold"
                                                                readonly>
                                                        </div>

                                                        <div class="flex items-center justify-start gap-3 mt-6">
                                                            <button type="reset"
                                                                data-modal-hide="edit-popup-modal-{{ $item->id }}"
                                                                class="border border-danger-600 hover:bg-danger-100 text-danger-600 text-base px-10 py-[11px] rounded-lg">
                                                                Cancel
                                                            </button>
                                                            <button type="submit"
                                                                id="btn-submit-pembayaran_{{ $item->id }}"
            ...
            forms.forEach(form => {
                ...
                function updateSisaSetelahPembayaran() {
                    ...
                    if (sisaSetelah < 0) {
                        ...
                        modalSisaSetelah.classList.remove('bg-success-50', 'border-success-200',
                            'text-success-600');
                        modalSisaSetelah.classList.add('bg-warning-50', 'border-warning-200',
                            'text-warning-600');

                        // Tombol tetap normal
                        btnSubmitPembayaran.disabled = false;
                        btnSubmitPembayaran.textContent = 'Simpan Pembayaran';
                        btnSubmitPembayaran.classList.remove('bg-success-600', 'border-success-600',
                        ...
                        modalSisaSetelah.classList.remove('bg-warning-50', 'border-warning-200',
                            'text-warning-600');
                        modalSisaSetelah.classList.add('bg-success-50', 'border-success-200',
                            'text-success-600');
                    ...
                    if (jumlahBayar > sisaTagihan && sisaTagihan > 0) {
                        ...
                        modalWarningText.textContent =
                            `⚠️ Pembayaran tidak boleh melebihi sisa tagihan (${formatRupiah(sisaTagihan)})`;
                ...
                const modalTrigger = document.querySelector(
                    `[data-modal-target="edit-popup-modal-${itemId}"]`);
                if (modalTrigger) {
                    modalTrigger.addEventListener('click', function() {
                        ...
                        if (sisaTagihan === 0) {
                            ...
                            btnSubmitPembayaran.classList.remove('btn-primary',
                                'border-primary-600');
                            btnSubmitPembayaran.classList.add('bg-success-600',
                                'border-success-600', 'cursor-not-allowed', 'opacity-60');
                            ...
                            btnSubmitPembayaran.disabled = false;
                            btnSubmitPembayaran.textContent = 'Simpan Pembayaran';
                            btnSubmitPembayaran.classList.remove('bg-success-600',
                                'border-success-600', 'cursor-not-allowed', 'opacity-60');
                            btnSubmitPembayaran.classList.add('btn-primary', 'border-primary-600');
                            updateSisaSetelahPembayaran();
                        }
