...
<x-page-table
    title="Data Pembayaran Membership"
    subtitle="Riwayat dan status tagihan keanggotaan."
>
    <x-data-table
        tableId="pembayaranMembership"
        :colspan="$colCount"
...
@if($isAdmin)
<x-modal id="bayar-membership-modal" title="Tambah Pembayaran Membership">
    <x-slot:body>
        <form id="bayarMembershipForm" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div class="col-span-12">
                    <label class="form-label">Kode Transaksi</label>
                    <input type="text" id="bayarKodeTrans" class="form-control bg-gray-50" readonly>
                </div>
                <div class="col-span-12 md:col-span-6">
                    <label class="form-label">Total Biaya</label>
                    <input type="text" id="bayarTotalBiaya" class="form-control bg-gray-50" readonly>
                </div>
                <div class="col-span-12 md:col-span-6">
                    <label class="form-label">Sudah Dibayar</label>
                    <input type="text" id="bayarSudahDibayar" class="form-control bg-gray-50" readonly>
                </div>
                <div class="col-span-12">
                    <label class="form-label font-semibold text-danger-600">Sisa Tagihan</label>
                    <input type="text" id="bayarSisaDisplay" class="form-control bg-danger-50 border-danger-200 text-danger-600 font-bold text-lg" readonly>
                    <input type="hidden" id="bayarSisaValue" value="0">
                </div>
                ...
                <div class="col-span-12">
                    ...
                    <input type="number" id="bayarJumlahBayar" name="jumlah_bayar" class="form-control" value="0" min="0" required>
                    <small id="bayarWarning" style="display:none; color:#dc3545; margin-top:4px;"></small>
                </div>
                <div class="col-span-12">
                    <label class="form-label">Sisa Setelah Pembayaran Ini</label>
                    <input type="text" id="bayarSisaSetelah" class="form-control bg-success-50 border-success-200 text-success-600 font-semibold" readonly>
                </div>
            </div>
        </form>
    </x-slot:body>
    <x-slot:footer>
        <button type="button" data-close-modal="bayar-membership-modal"
            class="border border-danger-600 hover:bg-danger-100 text-danger-600 text-base px-10 py-[11px] rounded-lg transition-colors">
            Cancel
        </button>
        <button id="bayarMembershipSubmitBtn" type="submit" form="bayarMembershipForm"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var bayarForm     = document.getElementById('bayarMembershipForm');
    var jumlahBayarEl = document.getElementById('bayarJumlahBayar');
    var sisaValueEl   = document.getElementById('bayarSisaValue');
    var sisaSetelahEl = document.getElementById('bayarSisaSetelah');
    var submitBtnEl   = document.getElementById('bayarMembershipSubmitBtn');
    var warningEl     = document.getElementById('bayarWarning');
    ...
    document.addEventListener('click', function (e) {
        var btn = e.target.closest('.open-bayar-modal');
        if (btn) {
            ...
            document.getElementById('bayarKodeTrans').value    = btn.dataset.kode;
            document.getElementById('bayarTotalBiaya').value   = formatRupiah(total);
            document.getElementById('bayarSudahDibayar').value = formatRupiah(dibayar);
            document.getElementById('bayarSisaDisplay').value  = formatRupiah(sisa);
            ...
            resetSubmitBtn();
            HexaModal.show('bayar-membership-modal');
        }
    });

    AjaxTable.init('pembayaranMembership', {
        url: '{{ route('pembayaran_membership.datatable') }}',
        colSpan: {{ $colCount }},
