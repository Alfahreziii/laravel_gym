        ...
        <div class="card border-0 overflow-hidden">
            ...
            <div class="card-body">
                <table id="selection-table" class="border border-neutral-200 rounded-lg border-separate">
                    ...
                    <tbody>
                        @foreach($specialisasis as $index => $specialisasi)
                        <tr>
                            <td class="whitespace-nowrap">{{ $index + 1 }}</td>
                            <td class="whitespace-nowrap">{{ $specialisasi->nama_specialisasi }}</td>
                            @role('admin')
                        ...
                        <!-- Modal Edit specialisasi -->
                        <div id="edit-popup-modal-{{ $specialisasi->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="rounded-2xl bg-white max-w-[800px] w-full">
                                <div class="py-4 px-6 border-b border-neutral-200 flex items-center justify-between">
                                    <h1 class="text-xl">Edit specialisasi</h1>
                                    <button data-modal-hide="edit-popup-modal-{{ $specialisasi->id }}" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center">
                                ...
                                <div class="p-6">
                                    <form action="{{ route('specialisasi.update', $specialisasi->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')

                                        <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                                            <div class="col-span-12">
                                                <label for="nama_specialisasi_{{ $specialisasi->id }}" class="inline-block font-semibold text-neutral-600 text-sm mb-2">Nama specialisasi :</label>
                                                <input type="text" id="nama_specialisasi_{{ $specialisasi->id }}" name="nama_specialisasi" class="form-control rounded-lg" value="{{ $specialisasi->nama_specialisasi }}" required>
                                            </div>
                                            <div class="col-span-12">
                                                <div class="flex items-center justify-start gap-3 mt-6">
                                                    <button type="button" data-modal-hide="edit-popup-modal-{{ $specialisasi->id }}" class="border border-danger-600 hover:bg-danger-100 text-danger-600 text-base px-10 py-[11px] rounded-lg">
                                                        Cancel
                                                    </button>
...
<!-- Modal Add specialisasi -->
<div id="popup-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="rounded-2xl bg-white max-w-[800px] w-full">
        <div class="py-4 px-6 border-b border-neutral-200 flex items-center justify-between">
            <h1 class="text-xl">Add New Specialisasi</h1>
        ...
        <div class="p-6">
            <form action="{{ route('specialisasi.store') }}" method="POST">
            @csrf
            @method('POST')

                <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                    <div class="col-span-12">
                        <label for="title" class="inline-block font-semibold text-neutral-600 text-sm mb-2">Nama Specialisasi : </label>
                        <input type="text" id="title" name="nama_specialisasi" class="form-control rounded-lg" placeholder="Masukkan Nama Specialisasi " required>
                    </div>
                    <div class="col-span-12">
                        <div class="flex items-center justify-start gap-3 mt-6">
