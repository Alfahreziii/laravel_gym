...
<div id="popup-modal" tabindex="-1" class="hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0">
    <div class="rounded-2xl bg-white max-w-[800px] w-full h-modal overflow-y-auto overflow-x-hidden">
        ...
        <div class="p-6">
            <form action="{{ route('anggota_membership.tambahPembayaran', $anggotaMembership->id) }}" method="POST" id="form-pembayaran">
            @csrf
            @method('POST')
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div class="col-span-12 md:col-span-6">
                        ...
                        <input type="text" id="modal_sudah_dibayar" class="form-control bg-gray-50" 
                            value="Rp {{ number_format($anggotaMembership->pembayaranMemberships->sum('jumlah_bayar'), 0, ',', '.') }}" readonly>
                    </div>
                    ...
                    <div class="col-span-12">
                        ...
                        <input type="text" id="modal_sisa_tagihan" class="form-control bg-danger-50 border-danger-200 text-danger-600 font-bold text-lg" 
                            value="Rp {{ number_format($anggotaMembership->total_biaya - $anggotaMembership->pembayaranMemberships->sum('jumlah_bayar'), 0, ',', '.') }}" readonly>
                        <input type="hidden" id="sisa_tagihan_value" value="{{ $anggotaMembership->total_biaya - $anggotaMembership->pembayaranMemberships->sum('jumlah_bayar') }}">
                    </div>
