...
@section('content')
    ...
    <!-- Statistik -->
    <div
        class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-4 3xl:grid-cols-4 gap-6 mx-auto mb-6">
        ...
        <div
            class="card shadow-none border border-gray-200 rounded-lg h-full bg-gradient-to-r from-orange-600/10 to-bg-white">
            <div class="card-body p-5">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                                ...
                                <tbody>
                                    @foreach ($activePackages as $mt)
                                        <tr class="{{ $mt->is_session_active ? 'table-warning' : '' }}">
                                            ...
                                            <td>
                                                <span class="badge bg-info">{{ $mt->sesi }} /
                                                    {{ $mt->paketPersonalTrainer->jumlah_sesi }}</span>
                                            </td>
                                            <td>
                            ...
                            <tbody>
                                @foreach ($memberTrainers as $mt)
                                    <tr>
                                        ...
                                        <td>
                                            <a title="Lihat Riwayat Gym"
                                                href="{{ route('trainer.member.history', $mt->id) }}"
                                        ...
                                        <td>
                                            @php
                                                ...
                                                $isActive =
                                                    $mt->tgl_mulai->format('Y-m-d') <= $today &&
                                                    $mt->tgl_selesai->format('Y-m-d') >= $today &&
                                                    $mt->sesi > 0;
                                                $isExpired =
                                                    $mt->tgl_selesai->format('Y-m-d') < $today || $mt->sesi <= 0;
                                            @endphp
