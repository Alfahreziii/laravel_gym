                    ...
                    <form action="{{ route('trainer.update', $trainer->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-12 gap-4">
                            ...
                            <div class="col-span-12 md:col-span-6">
                                ...
                                <select name="id_specialisasi"
                                    class="form-control @error('id_specialisasi') is-invalid @enderror" required>
                            ...
                            <div class="col-span-12 md:col-span-4">
                                ...
                                <select name="jenis_kelamin"
                                    ...
                                    <option value="Laki-laki"
                                        {{ old('jenis_kelamin', $trainer->jenis_kelamin) == 'Laki-laki' ? 'selected' : '' }}>
                                        Laki-laki</option>
                                    <option value="Perempuan"
                                        {{ old('jenis_kelamin', $trainer->jenis_kelamin) == 'Perempuan' ? 'selected' : '' }}>
                                        Perempuan</option>
                                </select>
                                @error('jenis_kelamin')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-span-12">
                                ...
                                <textarea name="alamat" class="form-control @error('alamat') is-invalid @enderror" rows="3" required>{{ old('alamat', $trainer->alamat) }}</textarea>
                                @error('alamat')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-span-12 md:col-span-6">
                                ...
                                <input type="date" name="tgl_gabung"
                                    class="form-control @error('tgl_gabung') is-invalid @enderror"
                            ...
                            <div class="col-span-12 md:col-span-6">
                                ...
                                <select name="status" class="form-control @error('status') is-invalid @enderror"
                                    ...
                                    <option value="aktif"
                                        {{ old('status', $trainer->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                    <option value="nonaktif"
                                        {{ old('status', $trainer->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif
                                    </option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-span-12">
                                ...
                                <select name="status_finger"
                                    ...
                                    <option value="2"
                                        {{ old('status_finger', $trainer->status_finger) == '2' ? 'selected' : '' }}>—
                                        Default (Tidak Ada Aksi) —</option>
                                    <option value="0"
                                        {{ old('status_finger', $trainer->status_finger) == '0' ? 'selected' : '' }}>🟢
                                        Enroll Fingerprint</option>
                                    <option value="1"
                                        {{ old('status_finger', $trainer->status_finger) == '1' ? 'selected' : '' }}>🔴
                            ...
                            <div class="col-span-12">
                                ...
                                @if ($trainer->user->photo)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/' . $trainer->user->photo) }}" alt="Current Photo"
                                    ...
                                    @foreach ($trainer->schedules as $index => $schedule)
                                        <div class="jadwal-item grid grid-cols-12 gap-4 mb-2">
                                            <div class="col-span-12 md:col-span-3">
                                                ...
                                                <select name="jadwal[{{ $index }}][day_of_week]"
                                                    class="form-control" required>
                                                    <option value="">-- Pilih Hari --</option>
                                                    @foreach (['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'] as $hari)
                                                        <option value="{{ $hari }}"
                                                            {{ $schedule->day_of_week == $hari ? 'selected' : '' }}>
                                                            {{ $hari }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-span-12 md:col-span-3">
                                                ...
                                                <input type="time" name="jadwal[{{ $index }}][start_time]"
                                                    class="form-control" value="{{ $schedule->start_time }}" required>
                                            </div>
                                            <div class="col-span-12 md:col-span-3">
                                                <label class="form-label">Jam Selesai</label>
                                                <input type="time" name="jadwal[{{ $index }}][end_time]"
                                                    class="form-control" value="{{ $schedule->end_time }}" required>
                                            </div>
                                            <div class="col-span-12 md:col-span-3 flex items-end">
                                                <button type="button"
                                                    class="btn-remove-jadwal text-danger-600 w-full focus:bg-danger-600 hover:bg-danger-700 border border-danger-600 hover:text-white focus:text-white focus:ring-4 focus:outline-none focus:ring-danger-300 font-medium rounded-lg text-base px-6 py-3">Hapus</button>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
