...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-4 3xl:grid-cols-4 gap-6 mx-auto">
                ...
                @if ($trainer->active_session)
                    <p class="font-medium text-sm text-neutral-600 mt-3 mb-0 flex items-center gap-2">
                        <span class="inline-flex items-center gap-1 text-success-600"><iconify-icon icon="bxs:up-arrow"
                                class="text-xs"></iconify-icon> Sedang melatih:</span>
        ...
        <div
            class="card shadow-none border border-gray-200 rounded-lg h-full bg-gradient-to-r from-cyan-600/10 to-bg-white">
            <div class="card-body p-5">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <tbody>
                            @foreach ($memberTrainers as $mt)
                                <tr class="{{ $mt->is_session_active ? 'table-warning' : '' }}">
                                    <td>
                                        <a title="Member Detail"
                                            href="{{ route('trainerlistmember.detail', $mt->id_anggota) }}">
                                            <strong class="text-primary-600">{{ $mt->anggota->name }}</strong><br>
                                            <small class="text-muted">{{ $mt->anggota->no_telp }}</small>
                                        </a>
                                    </td>
                                    <td>
                                        <a title="Lihat Riwayat Gym" href="{{ route('trainer.member.history', $mt->id) }}"
                                        ...
                                        @if ($mt->sisa_sesi > 0)
                                            @if (!$mt->is_session_active)
                                                @if (!$trainer->isTraining())
                                                    @if ($mt->is_checked_in)
                                                        <!-- Tombol buka modal -->
                                                        <button type="button" data-modal-target="start-session-modal"
                                                ...
                                                @endif
                                            @else
                                                <a href="{{ route('trainer.monitoring') }}" class="btn btn-sm btn-warning">
                                                    📊 Ke Monitoring
                                                </a>
                                            @endif
                                        @else
            ...
            <div class="p-6">
                <form id="endSessionForm" method="POST">
                    ...
                    <p class="text-neutral-700 text-base mb-4">
                        Apakah Anda yakin ingin menyelesaikan sesi training untuk <strong
                            id="endSessionMemberName"></strong>?
                    </p>
