...
var options = {
  series: [{
    name: "This month",
    data: window.dashboardData ? window.dashboardData.monthlyData : [] // default monthly
  }],
  chart: {
    height: 264,
    type: 'line',
...
document.querySelector("#chartFilter").addEventListener("change", function(e) {
  ...
  if (value === "Monthly") {
    chart.updateSeries([{ name: "Revenue", data: window.dashboardData.monthlyData }]);
    chart.updateOptions({
      xaxis: { categories: ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'] }
    });
  }
  else if (value === "Weekly") {
    chart.updateSeries([{ name: "Revenue", data: window.dashboardData.weeklyData }]);
    chart.updateOptions({
      xaxis: { categories: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'] }
    });
  }
  else if (value === "Yearly") {
    chart.updateSeries([{ name: "Revenue", data: [2000000, 2500000, 3000000, 4000000] }]); // contoh quarterly
    chart.updateOptions({
      xaxis: { categories: ['Q1','Q2','Q3','Q4'] }
    });
  }
  else if (value === "Today") {
    chart.updateSeries([{ name: "Revenue", data: [10000, 20000, 15000, 30000, 25000] }]); // contoh jam
    chart.updateOptions({
...
// =========================== Sales Statistic Line Chart End ===============================
  ...
  // ================================ Weekly Revenue Bar Chart Start ================================ 
  var weeklyOptions = {
    series: [{
      name: "Revenue",
      data: window.dashboardData ? window.dashboardData.weeklyData : []
    }],
    chart: {
      type: 'bar',
      height: 235,
  ...
  var barChart = new ApexCharts(document.querySelector("#barChart"), weeklyOptions);
  barChart.render();
  // ================================ Weekly Revenue Bar Chart End ================================ 
