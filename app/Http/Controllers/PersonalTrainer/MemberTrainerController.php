...
class MemberTrainerController extends Controller
{
    ...
    public function exportExcel(Request $request)
    {
        ...
        try {
            ...
            $rows = '';
            foreach ($memberTrainers as $i => $item) {
                $terbayar         = $item->pembayaranMemberTrainers->sum('jumlah_bayar');
                $sisaSesi         = $item->sesi;
                $totalSesi        = $item->paketPersonalTrainer->jumlah_sesi ?? 0;
                $tanggalBayarAwal = $item->pembayaranMemberTrainers->min('tgl_bayar');
                ...
                $rows .= '<tr>'
                    ...
                    . '<td>' . $this->exEsc($item->trainer->name ?? '-') . '</td>'
                    . '<td>' . $this->exEsc($item->paketPersonalTrainer->nama_paket ?? '-') . '</td>'
                    . '<td class="center">' . $sisaSesi . ' / ' . $totalSesi . '</td>'
            ...
            $sumTotalBiaya = $memberTrainers->sum('total_biaya');
            $sumTerbayar   = $memberTrainers->sum(fn($item) => $item->pembayaranMemberTrainers->sum('jumlah_bayar'));
            ...
            $html .= '<tr><td colspan="11"></td></tr>';
            $html .= '<tr>'
                . '<th>No</th><th>Kode Transaksi</th><th>Nama Anggota</th><th>Nama Trainer</th><th>Paket</th>'
                . '<th>Sesi</th><th>Tgl Bayar</th><th>Metode</th><th>Total Biaya</th>'
                . '<th>Terbayar</th><th>Status</th>'
                . '</tr>';
            $html .= $rows;
            $html .= '<tr class="grand-row">'
                . '<td colspan="8" class="center">TOTAL (' . $memberTrainers->count() . ' transaksi)</td>'
            ...
            $filename = 'Laporan_Member_Trainer_' . ucfirst($statusFilter) . '_' . date('Y-m-d_His') . '.xls';

            return $this->excelDownload($html, $title, $filename);
        } catch (\Exception $e) {
            Log::error('Gagal export Excel member trainer', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
