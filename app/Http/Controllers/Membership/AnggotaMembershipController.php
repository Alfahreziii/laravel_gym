...
class AnggotaMembershipController extends Controller
{
    ...
    /**
     * Export data anggota membership ke Excel (.xls) — filter sama seperti exportPdf.
     */
    public function exportExcel(Request $request)
    {
        ...
        try {
            ...
            $rows = '';
            foreach ($anggotaMemberships as $i => $item) {
                ...
                $statusBadge      = $item->status_pembayaran === 'Lunas' ? 'Lunas' : 'Belum Lunas';

                $rows .= '<tr>'
                    . '<td class="center">' . ($i + 1) . '</td>'
                    . '<td>' . $this->exEsc($item->kode_transaksi) . '</td>'
                    . '<td>' . $this->exEsc($item->anggota->name ?? '-') . '</td>'
                    . '<td>' . $this->exEsc($item->paketMembership->nama_paket ?? '-') . '</td>'
                    . '<td class="center">' . Carbon::parse($item->tgl_mulai)->format('d/m/Y') . '</td>'
                    . '<td class="center">' . Carbon::parse($item->tgl_selesai)->format('d/m/Y') . '</td>'
                    . '<td class="center">' . ($tanggalBayarAwal ? Carbon::parse($tanggalBayarAwal)->format('d/m/Y') : '-') . '</td>'
                    . '<td class="center">' . $this->exEsc($metodePembayaran) . '</td>'
                    . '<td class="num">Rp ' . $this->exNum($item->total_biaya) . '</td>'
                    . '<td class="num">Rp ' . $this->exNum($terbayar) . '</td>'
                    . '<td class="num">Rp ' . $this->exNum($sisa) . '</td>'
                    . '<td class="center">' . $statusBadge . '</td>'
                    . '</tr>';
            }

            $sumTotalBiaya = $anggotaMemberships->sum('total_biaya');
            $sumTerbayar   = $anggotaMemberships->sum(fn($item) => $item->pembayaranMemberships->sum('jumlah_bayar'));
            $sumSisa       = $sumTotalBiaya - $sumTerbayar;
            ...
            $html .= '<tr>'
                ...
                . '<td colspan="2" class="summary-label">Lunas / Belum Lunas</td><td colspan="2" class="summary-val">' . $totalLunas . ' / ' . $totalBelumLunas . '</td>'
                . '<td colspan="2" class="summary-label">Total Pendapatan</td><td colspan="2" class="summary-val">Rp ' . $this->exNum($totalPendapatan) . '</td>'
                . '</tr>';
            $html .= '<tr>'
                . '<td colspan="2" class="summary-label">Terbayar</td><td colspan="2" class="summary-val">Rp ' . $this->exNum($totalTerbayar) . '</td>'
                . '<td colspan="2" class="summary-label">Piutang</td><td colspan="2" class="summary-val">Rp ' . $this->exNum($totalPiutang) . '</td>'
                . '<td colspan="4"></td>'
                . '</tr>';
            $html .= '<tr><td colspan="12"></td></tr>';
            $html .= '<tr>'
                . '<th>No</th><th>Kode Transaksi</th><th>Nama Anggota</th><th>Paket</th><th>Tgl Mulai</th>'
                . '<th>Tgl Selesai</th><th>Tgl Bayar Awal</th><th>Metode Bayar</th><th>Total Biaya</th>'
                . '<th>Terbayar</th><th>Sisa</th><th>Status</th>'
                . '</tr>';
            $html .= $rows;
            $html .= '<tr class="grand-row">'
                . '<td colspan="8" class="center">TOTAL (' . $anggotaMemberships->count() . ' transaksi)</td>'
                . '<td class="num">Rp ' . $this->exNum($sumTotalBiaya) . '</td>'
                . '<td class="num">Rp ' . $this->exNum($sumTerbayar) . '</td>'
                . '<td class="num">Rp ' . $this->exNum($sumSisa) . '</td>'
                . '<td></td>'
                . '</tr>';
            $html .= '</table>';

            $filename = 'Laporan_Membership_' . ucfirst($statusFilter) . '_' . date('Y-m-d_His') . '.xls';

            return $this->excelDownload($html, $title, $filename);
        } catch (\Exception $e) {
            Log::error('Gagal export Excel membership', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
